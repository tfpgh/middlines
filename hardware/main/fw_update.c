...
static enum golioth_status fw_update_check_candidate(const uint8_t *hash, size_t img_size)
{
    ...
    if (_update_partition == NULL) {
        GLTH_LOGE(TAG, "No OTA partition available");
                                             ...
                                             void *arg)
{
    ...
    status = fw_update_handle_block(block_buffer,
                                    block_buffer_len,
                            ...
                            void *arg)
{
    ...
    parse_status = golioth_ota_payload_as_manifest(payload, payload_size, &_ota_manifest);
    xSemaphoreGive(_manifest_update_mut);

    if (parse_status != GOLIOTH_OK) {
        GLTH_LOGE(TAG,
                  "Failed to parse manifest: %d (%s)",
                  parse_status,
                  golioth_status_to_str(parse_status));
        return;
    }
                                          ...
                                          struct fw_update_component_context *ctx)
{
    ...
    new_component = golioth_ota_find_component(manifest, ctx->config.fw_package_name);
    if (new_component) {
        GLTH_LOGI(TAG,
...
static enum golioth_status fw_change_image_and_reboot(void)
{
    int countdown = 5;
    esp_err_t err;
...
static void fw_update_thread(void *arg)
{
    ...
    while (1) {
        ...
        start_time_ms = golioth_sys_now_ms();
        download_ctx.bytes_downloaded = 0;
        download_ctx.retries = 0;
        download_ctx.sha = golioth_sys_sha256_create();
        _update_handle = 0;

        if (download_ctx.sha == NULL) {
            GLTH_LOGE(TAG, "Failed to create SHA256 context for OTA download");
            backoff_increment(&_component_ctx);
            fw_download_failed(GOLIOTH_OTA_REASON_FIRMWARE_UPDATE_FAILED);
            continue;
        }

        retry_context.download_ctx = &download_ctx;
        retry_context.component_ctx = &_component_ctx;

        retry_timer_config.name = "";
        retry_timer_config.expiration_ms = FW_UPDATE_RESUME_DELAY_S * 1000;
        retry_timer_config.fn = block_retry_timer_expiry;
        retry_timer_config.user_arg = &retry_context;
        download_ctx.block_retry_timer = golioth_sys_timer_create(&retry_timer_config);

        if (download_ctx.block_retry_timer == NULL) {
            GLTH_LOGE(TAG, "Failed to create OTA retry timer");
        ...
        esp_err = esp_ota_end(_update_handle);
        _update_handle = 0;
        if (ESP_OK != esp_err) {
                                        ...
                                        const struct golioth_fw_update_config *config)
{
    ...
    _manifest_update_mut = xSemaphoreCreateMutex();
    _manifest_rcvd = xSemaphoreCreateCounting(1, 0);
    _download_complete = xSemaphoreCreateCounting(1, 0);

    if ((_manifest_update_mut == NULL) || (_manifest_rcvd == NULL) || (_download_complete == NULL)) {
        GLTH_LOGE(TAG, "Failed to allocate OTA synchronization primitives");
        return;
    }
